...
#figure(
  ...
  caption: [One neutral-density filter, fully assembled and disassembled: the printed container (with magnets for attaching the lid), the lid, and the assembled filter with the laser passing through it. Each filter's case measures #munit(5, "cm") #sym.times #munit(5, "cm") #sym.times #munit(0.5, "cm").],
)
...
#figure(
  ...
  caption: [The film holder: the mount that positions the photographic film at distance $D$ from the slit and keeps it flat during the (sometimes hours-long) exposure. Its wooden base measures #munit(11, "cm") #sym.times #munit(33, "cm").]
)

The holder also has holes for screws, so it can be fixed to the work surface and aligned with the rest of the apparatus. Its front section is detachable: a small block with #munit(0.4, "mm") tall tabs that the film slides into and is held flat by, with four magnets on its back that let it snap onto the vertical back plate. The film itself is not magnetic — only the block it slides into is. This split matters because the film has to be unpacked, unrolled, cut to size, and slid into the block in complete darkness before the laser can be switched on, which is very difficult to do by touch alone. With the room lit, the back plate is positioned and fixed to the work surface, aligned with the rest of the apparatus; the lights are then turned off, the film is unpacked and slid into the front block, and the block is snapped back onto the back plate by its own magnets — the film only has to be handled by touch, while the magnets make sure everything lands back in the correct position.
...
The non-attenuated exposure (Kentmere Pan 400 film, developed at an effective ISO of 400, exposed for about 40 s, with only $F_1 = F_2 lt.eq 0.8%$ engaged and $D = (4021 plus.minus 5)$ mm) was taken first, without attenuation, simply to confirm the apparatus produced a working diffraction pattern at all before moving on to the much longer attenuated acquisition. An unintended side effect of that choice is that the film saturates near the pattern's center, so the recorded optical density stops tracking the true light intensity there — which is what makes fitting the *positions* of the diffraction minima against fringe order the right approach for this image, since a minimum's location does not depend on how saturated the peaks around it are (this is, in fact, why the two exposures ended up needing two different analysis methods). Writing $x_"diff"(n)$ for the position of the $n$-th diffraction (envelope) minimum and $x_"int"(n)$ for the position of the $n$-th interference minimum,
...
The attenuated, single-photon exposure (Ilford Delta 3200 Professional film, push-processed to an effective ISO of 6400, exposed for about 8 hours, with the full five-filter stack engaged and $D = (4091 plus.minus 5)$ mm) was built up slowly enough to stay within the film's linear response range, so its *whole intensity profile* can instead be fit directly against the theoretical pattern shape, with no minima-picking needed — indeed, without strong attenuation the pattern would saturate just as the first exposure did, so working in the single-photon regime is not just conceptually motivated but also technically convenient here. For a double slit of width $a$ and separation $b$, observed at distance $D$, the theoretical relative intensity as a function of position $x$ from the pattern's center is
$ I_"rel"(x) = cos^2!(frac(pi b x, lambda D)) dot (frac(sin(frac(pi a x, lambda D)), frac(pi a x, lambda D)))^2. $
Writing $alpha = frac(lambda D, a)$ and $beta = frac(lambda D, b)$, and allowing for an overall scale $C$ to absorb the exposure's arbitrary normalization, gives the function actually fit to the data,
$ f(x) = C dot cos^2!(frac(pi x, beta)) dot (frac(sin(frac(pi x, alpha)), frac(pi x, alpha)))^2. $
$alpha$ and $beta$ (and hence $a$ and $b$) are obtained with a nonlinear least-squares fit of $f(x)$ to the measured intensity profile.
...
#figure(
  table(
    ...
    stroke: (x, y) => (
      ...
      bottom: if y == 0 or y == 3 { 1pt } else { 0pt },
    ),
    table.header(
    ...
    [Microscope],
    [direct measurement],
    [$(0.12 plus.minus 0.03)$ mm], [$(0.81 plus.minus 0.08)$ mm],

    [High intensity,\ minima positions],
    [Kentmere Pan 400, ISO 400 \ $T approx 40$ s, $F_1 = F_2 lt.eq 0.8%$ \ $D = (4021 plus.minus 5)$ mm],
    [$(0.13 plus.minus 0.04)$ mm], [$(0.8 plus.minus 0.2)$ mm],

    [Single photon,\ intensity fit],
    [Ilford Delta 3200, ISO 6400 \ $T approx 8$ h \ $F_1..F_4 lt.eq 0.8%$, $F_5 lt.eq 8%$ \ $D = (4091 plus.minus 5)$ mm],
    [$(0.15 plus.minus 0.01)$ mm], [$(0.85 plus.minus 0.03)$ mm],
  ),
